...
XtreamCredentials? credentialsFromUrl(String raw) {
  ...
  final hasM3u = type.contains('m3u');
  return XtreamCredentials(
    baseUrl: '${u.scheme}://${u.host}$port',
    username: user,
    password: pass,
    m3uUrl: hasM3u ? s : null,
  );
}
...
class XtreamClient {
  ...
  static const _ua = 'Sidimad-XtreamProv1/1.0 (Flutter)';
  static const _timeout = Duration(seconds: 25);

  // ---- plain M3U mode state (populated lazily by _ensureM3u) ----
  Future<void>? _m3uLoad;
  bool _apiAvailable = false; // true if Xtream API responded successfully
  ...
  Future<void> _loadM3u() async {
    ...
    if ((creds.epgUrl ?? '').isNotEmpty) {
      ...
      } catch (e) { debugPrint('[Xtream] EPG fetch failed (best-effort): $e'); }
    }
  }
  ...
  void _parseM3u(String body) {
    ...
    debugPrint('[Xtream] M3U body: ${lines.length} lines, first 3: ${lines.take(3).toList()}');
    final groups = <String>{};
    int id = 1;
    String? extinf;
    ...
    if (_m3uChannels.isEmpty) {
      debugPrint('[Xtream] WARNING: No channels parsed! First 5 non-empty lines:');
      for (final l in lines.where((l) => l.trim().isNotEmpty).take(5)) {
        debugPrint('  > ${l.trim()}');
      }
    }
  }
  ...
  /// Validate credentials. If m3uUrl is set, try Xtream API first for richer
  /// catalog; fall back to M3U-only if the API is unavailable.
  Future<Map<String, dynamic>> authenticate() async {
    ...
    // Local file: skip API, go straight to M3U parse.
    if ((creds.m3uUrl ?? '').startsWith('file://')) {
      await _ensureM3u();
      return {'auth': 1, 'username': creds.username};
    }
    // M3U mode: try Xtream API first for VOD/series/EPG, fall back to M3U live.
    try {
      final data = await _get({});
      if (data is Map && data['user_info'] != null && (data['user_info']['auth'] ?? 0) != 0) {
        _apiAvailable = true;
        debugPrint('[Xtream] API succeeded — using full catalog');
        return (data['user_info'] as Map).cast<String, dynamic>();
      }
    } catch (e) {
      debugPrint('[Xtream] API probe failed (expected for M3U): $e');
    }
    debugPrint('[Xtream] Falling back to M3U mode');
    _apiAvailable = false;
    await _ensureM3u();
    return {'auth': 1, 'username': creds.username};
  }

  bool get _useApi => !creds.isM3u || _apiAvailable;
  ...
  Future<List<Category>> vodCategories() async =>
      !_useApi ? const [] : _list(await _get({'action': 'get_vod_categories'}), Category.fromJson);
  Future<List<VodStream>> vodStreams(String? categoryId) async => !_useApi
      ? const []
  ...
  Future<List<Category>> seriesCategories() async =>
      !_useApi ? const [] : _list(await _get({'action': 'get_series_categories'}), Category.fromJson);
  Future<List<Series>> series(String? categoryId) async => !_useApi
      ? const []
