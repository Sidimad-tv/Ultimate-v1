...
class Store {
  static const _kActive = 'sidimad_active';
  static const _kProfiles = 'sidimad_profiles';
  ...
  /// Also migrates old "lumen_*" keys to the new "sidimad_*" keys.
  static Future<void> _migrate() async {
    if (_migrated) return;
    _migrated = true;
    if (!_useSecure) {
      // Desktop: migrate old lumen_* keys to sidimad_* keys.
      try {
        final p = await SharedPreferences.getInstance();
        for (final pair in [
          ('lumen_active', _kActive),
          ('lumen_profiles', _kProfiles),
        ]) {
          final old = p.getString(pair.$1);
          if (old != null && p.getString(pair.$2) == null) {
            await p.setString(pair.$2, old);
          }
          await p.remove(pair.$1);
        }
      } catch (_) {}
      return;
    }
